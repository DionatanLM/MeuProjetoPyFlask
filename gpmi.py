...
SQL_LISTA_CADASTRO = 'SELECT id, nome, sobrenome, senha, telefone from projeto'
SQL_CREATE_CADASTRO = 'INSERT into projeto (nome, sobrenome, senha, telefone) values (%s, %s,%s, %s))'

#Definicição das Classes que vai trabalhar
class Projeto:
    def __init__(self, nome, sobrenome, senha, telefone, id=None):
        self.id = id
        self.nome = nome
...
def create():
    ...
    projeto = Projeto(nome, sobrenome, senha, telefone)
    cursor = db.connection.cursor()
    cursor.execute(SQL_CREATE_CADASTRO, (projeto.nome, projeto.sobrenome, projeto.senha, projeto.telefone))
